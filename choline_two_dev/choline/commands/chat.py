...
######### so the way this works is reads the setup from the yaml then writes it to a hidden dir ..... 


def save_choline_from_git(url, local_filename=None):
    if local_filename is None:
        local_filename = url.split('/')[-1]  # Extract filename from URL if not provided
...
# URL of the raw content of the file you want to download
...
def get_choline_yaml_data(path='./choline.yaml'):
...
def main(cho_yaml_pth_or_model):

    if 'http' not in check_for_choline_txt: 
        file_url = "https://raw.githubusercontent.com/bdytx5/choline_templates/main/{}/choline.yaml".format(cho_yaml_pth_or_model)
        save_choline_from_git(file_url) # save to .choline 

    # choline_data = get_choline_json_data()
    choline_data = get_choline_yaml_data() if 'http' in cho_yaml_pth_or_model else get_choline_json_data(path=cho_yaml_pth_or_model)
...
def run(cho_yaml_pth_or_model):
    main(cho_yaml_pth_or_model)    